...
using PipeShot.Libraries.Start;
...
namespace PipeShot {
    ...
    public partial class EntryWindow {
        ...
        public void KillPipeShot() {
            Process.GetProcesses().Where(p => (p.ProcessName == "PipeShot" && p.Id != Process.GetCurrentProcess().Id)).ForEach(p => p.Kill());
        }
    }
}
