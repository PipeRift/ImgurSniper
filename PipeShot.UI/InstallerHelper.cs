...
namespace PipeShot.UI {
    public class InstallerHelper {
        ...
        public void Clean() {
            try {
                string file = Path.Combine(_updateZipPath, "PipeShotSetup.zip");
                string directory = Path.Combine(_updateZipPath, "PipeShotInstaller");
        ...
        //Download the PipeShot Archive from github
        public void Update(Panel panel) {
            ...
            using (WebClient client = new WebClient()) {
                ...
                client.DownloadFileAsync(
                    new Uri(@"https://github.com/piperift/PipeShot/blob/master/Downloads/PipeShotSetup.zip?raw=true"),
                    file);
            }
        }

        public static void KillPipeShot(bool killSelf) {
            List<Process> processes =
                new List<Process>(Process.GetProcesses().Where(p =>
                (p.ProcessName.Contains("PipeShot")) &&
                (!p.ProcessName.Contains("Setup"))));

            foreach (Process p in processes) {
                try {
                    if (p.Id != Process.GetCurrentProcess().Id) {
                        //p.CloseMainWindow();    //does not work for PipeShot - no main window
                        p.Kill();
                        p.WaitForExit();
                    }
                } catch {
        ...
        public static void StartPipeShot() {
            List<Process> processes =
                new List<Process>(Process.GetProcesses().Where(p =>
                    (p.ProcessName.Contains("PipeShot")
            ...
            if (processes.Count < 1) {
                Process p = new Process {
                    StartInfo = {
                        FileName = Path.Combine(ConfigHelper.InstallDir, "PipeShot.exe"),
                        Arguments = "-autostart"
                    }
                };
                p.Start();
        ...
        private async void DownloadCompleted(object sender, AsyncCompletedEventArgs e) {
            string file = Path.Combine(_updateZipPath, "PipeShotSetup.zip");
            string extractTo = Path.Combine(_updateZipPath, "PipeShotInstaller");
            string installerPath = Path.Combine(extractTo, "PipeShotSetup.exe");

            if (!File.Exists(file)) {
                _error.Show(strings.couldNotDownload,
                    TimeSpan.FromSeconds(5));
                await Task.Delay(1000);
                Process.Start("https://github.com/piperift/PipeShot");
            } else {
        ...
        //Extract the downloaded PipeShot Messenger Archive
        ...
        public static void Autostart(bool enableAutostart) {
            try {
                string file = Path.Combine(ConfigHelper.InstallDir, "PipeShot.exe");
                ...
                using (RegistryKey baseKey =
                    ...
                    if (baseKey != null) {
                        if (enableAutostart) {
                            baseKey.SetValue("PipeShot", path);
                            StartPipeShot();
                        } else {
                            baseKey.DeleteValue("PipeShot");
                        }
                    }
                }
            } catch {
        ...
        public static async Task<bool> CheckForUpdates(MainWindow window, bool forceSearch) {
            try {
                ...
                if (forceSearch || DateTime.Now - lastChecked > TimeSpan.FromDays(1) || updateAvailable) {
                    ...
                    GitHubClient github = new GitHubClient(new ProductHeaderValue("PipeShot"));
                    IReadOnlyList<GitHubCommit> commitsRaw = await github.Repository.Commit.GetAll("piperift",
                        "PipeShot");
                    //All Commits where a new PipeShot Version is available start with "R:"
